...
TEST(test_distance, player_facing_north_sees_a_wall_in_half_a_tile_distance)
{
	...
	t_matrix dir;
	generate_direction_vector(&game_obj.orientation, &dir);

	float result = calc_distance_to_obstacle(&dir, &game_obj);
...
TEST(test_distance, calculate_distance_to_closest_wall)
{
	...
	t_map *map = parse_map("test/assets/dummy_map_player_one_field_from_north");
	t_game_obj game_obj = init_game_obj(map);
	game_obj.orientation = euler2rot(0., 0., 135.);
	generate_direction_vector(&game_obj.orientation, &dir);
...
TEST(test_distance, looking_up_will_find_ceiling)
{
	...
	t_map *map = parse_map("test/assets/dummy_map_player_one_field_from_north");
	t_game_obj game_obj = init_game_obj(map);
	game_obj.orientation = euler2rot(0., 90., 0.);
	generate_direction_vector(&game_obj.orientation, &dir);
